<template>
  <div>
    <HeaderBar v-if="sessionReady && role" :token="token" :role="role" :logo-src="logoSrc"
      ...
      @goto-admin-add-task="gotoAddTaskFromHeader" @home="onHome" @open-settings="showSettings=true" @open-admin-tools="openAdminTools" />
    <ToastContainer />
    <div class="p-6">
    ...
    <AdminTools v-if="showAdminTools" :token="token" @close="showAdminTools=false" />
  </div>
</template>
...
export default {
  components: { Login, AdminDashboard, CalendarView, MonthSchedule, HeaderBar, ToastContainer, BlockedDaysModal, AdminTools: () => import('./components/AdminTools.vue') },
  data(){
    return {
  ...
  methods: {
    ...
    openAdminTools(){ this.showAdminTools = true },
    onFilter(list){
